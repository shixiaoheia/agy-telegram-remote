...
步骤 3/3：Google 账号授权
• 若已有历史有效授权，将自动识别并复用；
• 首次授权终端将直接输出授权网址 → 浏览器登录 Google → 复制授权码粘贴回车即可完成，无需手动输入 /exit。
...
| 📎 **直接发送截图/文件** | 附件分析 | 截图、日志或常见代码文件，单文件最大 10MB | 白名单用户 | 附件会被临时放入本次任务的私有工作目录，agy 自动读取分析，任务结束即清理 |
| 🧠 `/model` `[别名\|名称\|default]` | 模型与思考强度 | 可选别名或全名，缺省呼出按钮 | 白名单用户 | 先选择模型，再通过行内按钮选择思考强度；支持官方全量 14 种模型 |
...
| 📚 `/history` | 任务历史 | 无参数 | 白名单用户 | 展示最近 10 条任务的标题、状态、耗时；点击对应按钮可打开完整结果 |
...
- **🔘 两步选择与自动收起**：发送 `/model`，先从行内键盘选择模型；Gemini 系列确认后会原地切换为思考强度按钮，选择完成后同一条消息收起为“已切换至：模型｜思考强度：档位”，不会留下多张菜单卡片。
- **固定强度模型**：Claude 与 GPT-OSS 的思考配置由模型自身固定，agy 不支持再传 `--effort`。选择这类模型时机器人会自动清除旧强度设置并直接显示“思考强度：模型内置”，避免无效选择报错。
- **⌨️ 快捷别名与指令输入**：
...
1. **高信任 Root 原生模式**：程序专为个人独立 VPS 设计，以 root 账户常驻运行，并默认开启 `--dangerously-skip-permissions`（自动审批）。白名单用户享有主机 root 级别的完全控制权限，因此**严禁向不可信用户开放白名单**。所有配置文件、持久化状态库均强制设置为 `0600` root 专用权限，严禁全局可读。
2. **明确的主管理员权限（`owner_id`）**：白名单首位 ID 为主管理员，严格独占 `/restart` 进程重载与 `/whitelist` 动态授权权限；基础配置管理员享受防误踢保护。
3. **供应链加固与不可变升级**：推荐生产环境使用 `bash install.sh --ref <commit-sha>` 固定不可变版本升级；远端官方安装脚本受完整性与结构特征校验约束，避免执行损坏或不可信脚本。
4. **工作目录边界与路径安全**：工作目录默认为 `/root`（或用户自定义子路径），内置严格防目录遍历与非法符号链接校验，防止越权读写外部敏感文件。
5. **强白名单与私聊防护**：系统仅响应白名单内的 Telegram 私聊消息，未授权用户或群聊消息一律直接静默忽略或告警拦截。
6. **单任务互斥执行**：共享工作目录下同一时刻仅允许运行一个任务，杜绝并发竞争写引发的文件冲突。
...
本项目包含完备的离线自动化测试套件（含 189 项全面断言测试），覆盖语法、沙箱权限、参数注入防御、状态持久化、会话记忆、Token 用量解析、系统监控、白名单运维、行内键盘回调与 OAuth 包装器：
