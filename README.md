...
- 🔘 **Telegram 行内按钮一键直切模型（Inline Keyboard）**：
  - 发送 `/model` 弹出交互式按钮面板，手指轻点即刻切换目标模型；
...
| 📊 `/status` | 状态监控 | 查看任务运行状态、当前生效模型、活动会话及工作区磁盘剩余空间 |
...
本项目包含完备的离线自动化测试套件（含 166 项全面断言测试），覆盖语法、沙箱权限、参数注入防御、状态持久化、会话记忆、Token 用量解析与行内键盘切换：
